...
//les constantes
var main = document.querySelector("main"); //les fonctions utilies pour le projet

var utils = {
  pageContent: function pageContent(title, content, btn) {
    document.querySelector("h1").innerHTML = title;
    main.innerHTML = content;
    main.classList = "";
    document.querySelector(".btn-container").innerHTML = btn;
  },
  getDogsInformation: function getDogsInformation() {
    _axios.default.head();

    _axios.default.get('https://fakerapi.it/api/v1/custom?_quantity=5&name=name&race=pokemon&image=image&description=text').then(function (response) {
      main.innerHTML = '';
      ...
      response.data.data.forEach(function (element) {
        ...
        carteEl.innerHTML = "\n                        <img src=\"".concat(element.image, "\" class=\"card-img-top\" widht=\"100px\">\n                        <div class =\"card-body\">\n                        <h5 class =\"card-title\">").concat(element.name, "</h5>\n                        <p class =\"card-text\">\n                        <strong>Race :</strong>").concat(element.race, "<br/>\n                        <strong>Description</strong>").concat(element.description, "\n                        </p>\n                        </div>\n                    \n                    ");
        rowEl.appendChild(carteEl);
      });
      main.appendChild(rowEl);
    }).catch(function (error) {
      main.innerHTML = "Error : ".concat(error.message);
...
var page = {
  lobby: function lobby() {
    utils.pageContent("Liste des Chiens sur le site <i id='reboot' class='fas fa-dog'></i>", null, "<button id='start' class='mx-5 btn btn-light text-uppercase fw-bold'> voir la liste <i class='fas fa-play-circle'></i></button>");
  },
  routine: function routine() {
    utils.pageContent("Routine", "Exercice avec chrono", null);
  },
  finish: function finish() {
    utils.pageContent("C'est terminé !", "<button id='start' class='mx-5 btn btn-light'>Recommencer</button>", "<button id='reboot' class='mx-5 btn btn-light'>Réinitialiser <i class='fas fa-times-circle'></i> </button>");
  }
};
